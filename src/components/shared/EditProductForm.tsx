...
export function EditProductForm({ product }: { product: Product }) {
  ...
  // preview: object URL for new uploads, or /api/blob proxy for existing
  const [photos, setPhotos] = useState<{ url: string; preview: string; name: string }[]>(
    (product.photoUrls as string[] ?? []).map((url) => ({
      url,
      preview: `/api/blob?url=${encodeURIComponent(url)}`,
      name: url.split("/").pop() ?? "photo",
    }))
  );
  const [uploading, setUploading] = useState(false);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [tags, setTags] = useState<string[]>((product.tags as string[]) ?? []);
  ...
  const priceDisplay = product.price != null ? (product.price / 100).toString() : "";
      ...
      <div className="space-y-4">
        ...
        <Card title="ข้อมูลสินค้า" desc="ชื่อ รหัส ราคา และหมวดหมู่">
          <Field label="ชื่อสินค้า" required>
            <input name="name" required defaultValue={product.name} placeholder="เช่น Trusme Vitamin C Serum 30ml" className={inputCls} />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="SKU / รหัสสินค้า">
              <input name="sku" defaultValue={product.sku ?? ""} placeholder="TC-SER-001" className={cn(inputCls, "font-mono")} />
            </Field>
            <Field label="ราคา (บาท)">
              <input name="price" type="number" min="0" step="0.01" defaultValue={priceDisplay} placeholder="890" className={inputCls} />
            </Field>
          </div>
          ...
          <Field label="คำอธิบายสินค้า" hint="AI จะใช้ข้อมูลนี้ในการสร้าง Content">
            <textarea name="description" rows={4} defaultValue={product.description ?? ""} placeholder="จุดเด่น ส่วนผสม วิธีใช้ ฯลฯ" className={inputCls} />
          </Field>
        </Card>
          ...
          >
            {uploading ? (
              ...
              <>
                ...
                <p className="text-sm text-foreground font-medium">คลิกเพื่ออัปโหลดรูปเพิ่ม</p>
